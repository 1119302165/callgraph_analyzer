"""
TypeScript AST analyzer for call graph generation using tree-sitter.
"""
import logging
import os
import traceback
...
class TreeSitterTSAnalyzer:
    def __init__(self, file_path: str, content: str, repo_path: str = None):
        ...
        try:
            ts_language = get_parser("typescript")
    ...
    def _traverse_for_functions(self, node) -> None:
        if node.type in ["class_declaration", "abstract_class_declaration", "interface_declaration"]:
            ...
            # Handle methods inside class body
            method_name = self._get_method_name(node)
            if method_name:
                method_node = self._create_method_node(node, method_name)
                if method_node and self._should_include_function(method_node):
                    self.nodes.append(method_node)
                    method_key = f"{self._get_module_path()}.{method_node.class_name}.{method_node.name}"
        ...
        for child in class_body.children:
            if child.type == "method_definition":
                ...
                if method_name:
                    ...
                    if method_node:
                        self.top_level_nodes[method_key] = method_node
            elif child.type == "public_field_definition" or child.type == "private_field_definition":
    ...
    def _get_field_name(self, field_node) -> Optional[str]:
        ...
        if field_node.type not in ["public_field_definition", "private_field_definition"]:
            return None
    ...
    def _create_method_node(self, node, method_name: str, class_name: str = None) -> Optional[Node]:
    ...
    def _extract_function_declaration(self, node) -> Optional[Node]:
        try:
            # Look for the identifier in the function declaration
            name_node = None
            for child in node.children:
    ...
    def _extract_exported_function(self, node) -> Optional[Node]:
        ...
        try:
            # Look for function declaration inside export statement
            for child in node.children:
                if child.type == "function_declaration":
                    ...
                    if func:
                        ...
                        if "export default" in export_text and "function (" in export_text:
                            func.name = "default"
                    return func
                elif child.type == "identifier":  # export identifier
        ...
        try:
            for child in node.children:
                if child.type == "variable_declarator":
                    ...
                    # Look for arrow function, function expression, or call expression
                    func_node = None
                    for subchild in child.children:
                        if subchild.type in ["arrow_function", "function_expression", "call_expression"]:
                            func_node = subchild
                            break
    ...
    def _extract_parameters(self, node) -> List[str]:
        parameters = []
        # Look for formal_parameters or primary_parameters
        params_node = self._find_child_by_type(node, "formal_parameters")
        if not params_node:
            params_node = self._find_child_by_type(node, "primary_parameters")
        ...
        if params_node:
            for child in params_node.children:
                if child.type == "identifier":
                    ...
                    param_id = self._find_child_by_type(child, "identifier")
                    if param_id:
                        parameters.append(self._get_node_text(param_id))
        return parameters
            ...
            for pattern in type_patterns:
                ...
                for match in matches:
                    ...
                    for base_type in base_types:
                        if base_type and not self._is_builtin_type_ts(base_type):
                            caller_id = f"{self._get_module_path()}.{caller_name}"
    ...
    def _is_builtin_type_ts(self, name: str) -> bool:
        """Check if type name is a TypeScript built-in type."""
        builtin_types = {
            ...
            # Common TypeScript generic parameters
            "T", "U", "V", "K", "P", "R", "E",
            
            # TypeScript specific types
            "Readonly", "Pick", "Record", "Exclude", "Omit", "Partial", "Required", "NonNullable",
